...
export const parseStatement = async (req, res) => {
  try {
    ...
    const prompt = `Analyze this bank statement. Return ONLY a JSON array, no markdown:
[{"description":"...","amount":0.00,"type":"income|expense","category":"Food|Transport|Shopping|Entertainment|Health|Utilities|Salary|Other","date":"YYYY-MM-DD"}]`;
    ...
    const transactions = JSON.parse(result.response.text().trim());
...
export const getInsights = async (req, res) => {
  try {
    ...
    const model = genAI.getGenerativeModel({ model: 'gemini-1.5-flash' });
    const result = await model.generateContent(
      `Transactions: ${JSON.stringify(transactions)}. Give 3 actionable insights as JSON only:
[{"title":"...","insight":"...","tip":"..."}]`
    );
    res.json(JSON.parse(result.response.text().trim()));
  } catch (e) { res.status(500).json({ error: e.message }); }
};
